...
function usage(message) {
  ...
  console.error(`
  Usage: node scripts/deploy.mjs <target> [--yes]
...
if (target.confirm && !flags.includes('--yes')) {
...
console.log(`  url      ${SITE}/${target.dest === '.' ? '' : target.dest + '/'}`)
console.log(`  source   ${branch} @ ${describe}`)

if (dirty) {
  console.log(`\n  Note: the working tree has uncommitted changes, so this build`)
  console.log(`  will not correspond to any commit. Deploying anyway.`)
}
...
console.log(`  Done: ${SITE}/${target.dest === '.' ? '' : target.dest + '/'}\n`)
